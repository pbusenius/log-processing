...
def open_log(file: str) -> pl.DataFrame:
    with open(file, "r") as file:
        ...
        for line in lines:
            ...
            if x is not None:
                if x["type"] ==  "password":
                    print(f"{x["date"]},{x["user"]},{x["type"]},{x["remote_user"]},{x["service"].split(":")[0]},{x["ip"]},{x["port"]}")
                else:
                    print(f"{x["date"]},{x["user"]},{x["type"]},{x["remote_user"]},{x["service"].split(":")[0]},{x["ip"]},{x["port"]},{x["rsa"]}")
